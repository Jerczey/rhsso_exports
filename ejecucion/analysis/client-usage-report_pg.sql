-- Objetivo 2: clientId en uso vs sin actividad — PostgreSQL (RHSSO / Keycloak)
-- Ejemplo podman:
--   podman exec -i ${PG_CONTAINER} psql -U ${PG_USER} -d ${PG_DB} < client-usage-report_pg.sql
-- O usar: ./client-usage-report.sh  (con DB_TYPE=pg y config.env)

\echo '=== Client usage by realm (sessions, offline tokens, events) ==='
...
active_sessions AS (
  ...
  JOIN user_session us ON cs.session_id = us.id
...
SELECT
  ...
  CASE
    ...
    THEN to_timestamp(ue.last_user_event_ms / 1000.0)
    ELSE NULL
...
\echo ''
\echo '=== Realms without user events enabled ==='
